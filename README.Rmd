...
- \textbf{Approximate approach through the seasonal B-spline (sB-spline) approximation:} The sGP is approximated as $\tilde{g}_k(x)$ using a set of sB-spline basis functions, with a set of Gaussian weights.
...
To use the sB-spline approach, the sGP is approximated as $\tilde{g}_k(x)$ using a set of sB-spline basis functions with a set of Gaussian weights.
